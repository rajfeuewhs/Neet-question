...
def save_test():
    try:
        d = request.json
        sub = d['subject'].lower().strip()
        chap = d['chapter'].strip() or "Direct_Tests"
        t_name = d['test_name'].strip()
        sc, sn = chap.replace(' ','_'), t_name.replace(' ','_')
        
        # Upload question file
        github_upload(f"data/{sub}/{sc}/{sn}.json", json.dumps(d['questions'], indent=2), f"Add {t_name}")
        
        # Update config with unlock_at (Timer logic)
        rc = requests.get(f"{RAW_BASE_URL}config.json?v={int(time.time())}")
        conf = rc.json() if rc.status_code == 200 else {}
        if sub not in conf: conf[sub] = {}
        if chap not in conf[sub]: conf[sub][chap] = []
        conf[sub][chap] = [t for t in conf[sub][chap] if t['name'] != t_name]
        conf[sub][chap].append({
            "name": t_name, 
            "file": f"{sub}/{sc}/{sn}", 
            "unlock_at": d.get('unlock_at', "") # Wapas timer joda
        })
